...
def testModel(model_path,test_path,save_path):
    modelUnet = unet(pretrained_weights=model_path)
    testGener = testGenerator(test_path)
    testPredictions = modelUnet.predict_generator(testGener,noImagesTest,verbose=1)
...
for cvI in range(0,NoCV):
    ...
    noImagesTrain=len(glob.glob(os.path.join(str(Path(dataDir,'train','image')), "*.png")))
    noImagesValidate=len(glob.glob(os.path.join(str(Path(dataDir,'validate','image')), "*.png")))
    noImagesTest = len(glob.glob(os.path.join(str(Path(dataDir,'test','crops','image')), "*.png")))
    trainGener = trainGenerator(batch_size,str(Path(dataDir,'train')),'image','label',data_gen_args,save_to_dir = None)
    valGener = validateGenerator(batch_size,str(Path(dataDir,'validate')), 'image', 'label', save_to_dir=None)
    modelUnet = unet(learningRate=learning_rateI,decayRate=decayI)
    ...
    history1=modelUnet.fit_generator(trainGener,epochs=NumEpochs,steps_per_epoch=int(noImagesTrain/batch_size),
                                    validation_data=valGener,validation_steps=noImagesValidate,callbacks=[model_checkpoint])
    ...
    for subItest in range(0, len(subjectNos_cvI_testing)):
        sliceNos = hemorrhageDiagnosisArray[hemorrhageDiagnosisArray[:, 0] == subjectNos_cvI_testing[subItest], 1]
        sliceInds = np.where(hemorrhageDiagnosisArray[:, 0] == subjectNos_cvI_testing[
        ...
        for sliceI in range(sliceNos.size):
            ...
            for i in range(noMoves):
                for j in range(noMoves):
                    windowI = io.imread(Path(SaveDir_crops_cv,str(subjectNos_cvI_testing[subItest])
                                  + '_' + str(sliceI) +'_'+ str(counterCrop)+ '.png'), as_gray=True)
                    windowI = windowI / 255
                    CTslicePredict[int(i * imageLen / (noMoves + 1)):int(i * imageLen / (noMoves + 1) + windowLen),
                        int(j * imageLen / (noMoves + 1)):int(j * imageLen / (noMoves + 1) + windowLen)]= CTslicePredict[int(i * imageLen / (noMoves + 1)):int(i * imageLen / (noMoves + 1) + windowLen),
                        int(j * imageLen / (noMoves + 1)):int(j * imageLen / (noMoves + 1) + windowLen)]+windowI
                    windowOcc[int(i * imageLen / (noMoves + 1)):int(i * imageLen / (noMoves + 1) + windowLen),
                        int(j * imageLen / (noMoves + 1)):int(j * imageLen / (noMoves + 1) + windowLen)]= windowOcc[int(i * imageLen / (noMoves + 1)):int(i * imageLen / (noMoves + 1) + windowLen),
                        int(j * imageLen / (noMoves + 1)):int(j * imageLen / (noMoves + 1) + windowLen)]+1
                    counterCrop = counterCrop + 1
            CTslicePredict=CTslicePredict / windowOcc *255
            img=CTslicePredict.astype(int)
            imsave(Path(SaveDir_full_cv,str(subjectNos_cvI_testing[subItest])
                                  + '_' + str(sliceI) + '.png'), img)  # 29: to take only the image name that was read before
            ...
            imsave(Path(SaveDir_cv,str(subjectNos_cvI_testing[subItest])
                        + '_' + str(sliceI) + '.png'), img)  # 29: to take only the image name that was read before
